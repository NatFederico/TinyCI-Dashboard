...
export class HttpHeaderInterceptor implements HttpInterceptor {
    ...
    constructor(
        ...
        this.authState.pipe(select(fromAuthSelectors.getUser), filter((user) => user != null)).subscribe((x) => { this.user = x; });
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        if (request.url.indexOf('./assets/') === -1) {
            ...
            if (!headers.has('unitn-referenceid')) {headers = headers.set('unitn-referenceid', sessionStorage.getItem(environment.storage.keys.referenceId)); }
            if (!headers.has('api-version')) {headers = headers.set('api-version', environment.apiVersion); }
